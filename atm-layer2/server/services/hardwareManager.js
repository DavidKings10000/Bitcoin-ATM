...
const createHardwareManager = ({ onStateChange = () => {}, onEvent = () => {} } = {}) => {
  ...
  const normalizeStatus = (status, endpoint) => {
    ...
    return {
      ...
      transport: status.transport || "sidecar",
      ...
      payoutCapacity: Number(status.payoutCapacity || 0),
      payoutDenominations: Array.isArray(status.payoutDenominations) ? status.payoutDenominations : [],
